...
**6. WebView2 런타임 오류 — registry 미등록 환경 (회사 PC / LTSC / GPO 차단)**
- 증상: 앱 실행 시 "You may have it installed on another user account, but it is not available for this one." 또는 비슷한 WebView2 다이얼로그
- 우선 시도: release 페이지의 `MicrosoftEdgeWebView2RuntimeInstallerX64.exe` 를 **관리자 권한** 으로 실행
- admin 권한이 아예 없는 환경 (LTSC 1809 회사 PC 등):
  1. release 페이지에서 `webview2-fixed-runtime-*.zip` 다운로드 (~170MB)
  2. zip 압축 해제 후 안에 들어있는 `EBWebView` 폴더 통째로 **Anything 설치 폴더 (`%LocalAppData%\Programs\Anything\`)** 의 최상단에 복사 → 최종 경로가 `%LocalAppData%\Programs\Anything\EBWebView\msedgewebview2.exe` 가 되도록
  3. Anything 재시작
- registry 등록 / admin 권한 / GPO 정책과 무관하게 동작합니다 (v2.6.3+).
